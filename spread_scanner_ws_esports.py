"""
Real-time WebSocket spread scanner for cross-venue arbitrage monitoring (ESPORTS ONLY).
...
This version filters to ONLY esports/gaming markets for live testing.

Architecture:
    1. Bootstrap — REST fetch of initial orderbooks + equivalent pairs (esports only)
...
Usage:
    python spread_scanner_ws_esports.py                          # 1-hour default
    python spread_scanner_ws_esports.py --duration 86400         # 24-hour session
    python spread_scanner_ws_esports.py -d 3600 -o report.png   # custom output
...
logger = logging.getLogger("spread_ws_esports")


# ── Esports filtering patterns ──────────────────────────────
# Compiled regex patterns for efficient esports market detection
_ESPORTS_KEYWORDS = [
    ...
    r"\bcs2\b", r"\bcounter\s+strike\s+2\b", r"\bcs\s+2\b",
    r"\bmap\s+\d+\b", r"\bgame\s+\d+\b", r"\bmatch\s+\d+\b",
    r"\bwin\s+map\b", r"\bwin\s+game\b", r"\bwin\s+match\b",
    r"\bpgl\b", r"\besl\b", r"\bblast\b", r"\bepicenter\b",
    r"\bchallengers\b", r"\bacademy\b", r"\brolster\b", r"\bt1\b",
    r"\bloud\b", r"\bfuria\b", r"\bmongolz\b", r"\bmouz\b",
    r"\bteam\s+vitality\b", r"\bteam\s+falcons\b", r"\bnatus\s+vincere\b",
    r"\bnavi\b", r"\bgiantx\b", r"\bkarmine\b", r"\bcorp\b",
]

_ESPORTS_PATTERNS: List[Pattern[str]] = [
    re.compile(pattern, re.IGNORECASE) for pattern in _ESPORTS_KEYWORDS
]


def _is_esports_pair(pair: EquivalentPair) -> bool:
    """
    Check if a pair is esports-related by examining titles.
    
    Args:
        pair: EquivalentPair to check
        
    Returns:
        True if either market title matches esports keywords
    """
    combined_text = f"{pair.title_a} {pair.title_b}".lower()
    
    # Check if any esports pattern matches
    for pattern in _ESPORTS_PATTERNS:
...
def generate_graph(
    ...
    output_path: str = "spread_divergence_esports.png",
    duration_label: str = "",
) -> str:
    """
    ...
    fig.suptitle(
        f"Cross-Venue Spread Scanner — Live WebSocket Monitor (ESPORTS ONLY)\n"
...
def print_live_summary(
    ...
    print("=" * 120)
    print(
        f"  LIVE SPREAD MONITOR (ESPORTS ONLY)  |  "
...
def print_final_report(
    ...
    print()
    print("=" * 130)
    print("  FINAL SESSION REPORT (ESPORTS ONLY)")
    print("=" * 130)
    print(f"  Duration: {up}")
...
async def run_ws_scanner(
    ...
    """
    Full lifecycle of the WebSocket spread scanner (ESPORTS ONLY).

    1. Fetch equivalent pairs from PostgreSQL.
    2. Filter to esports markets only.
    3. Bootstrap orderbooks via REST.
    4. Resolve Polymarket token IDs.
    5. Start Kalshi + Polymarket WS feeds as background tasks.
    6. Print periodic summaries.
    7. On duration expiry or SIGINT, generate graph + print report.
    """
    ...
    all_pairs = await fetch_equivalent_pairs(dsn)
    logger.info("Found %d total equivalent pairs", len(all_pairs))
    
    # ── 2. Filter to esports only ────────────────────────────
    pairs = [p for p in all_pairs if _is_esports_pair(p)]
    logger.info("Filtered to %d esports pairs", len(pairs))
    
    if not pairs:
        print("\nNo esports equivalent pairs in the database.")
        logger.warning("No esports pairs found — check database or run esports discovery")
        return

    manager = LiveOrderbookManager(pairs)

    # ── 3. partition by venue ────────────────────────────────
    ...
    logger.info("Tracking %d Kalshi tickers + %d Polymarket markets (esports only)", 
                len(kalshi_tickers), len(poly_mids))

    # ── 4. bootstrap + WS feeds ──────────────────────────────
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Real-time WebSocket spread scanner for cross-venue arbitrage (ESPORTS ONLY).",
    )
    parser.add_argument(
        "-d", "--duration",
        type=int,
    ...
    parser.add_argument(
        "-o", "--output",
        type=str,
        default="spread_divergence_esports.png",
        help="Output graph file path (default: spread_divergence_esports.png)",
    )
    parser.add_argument(
        "-s", "--summary-interval",
        type=int,
    ...
    logger.info(
        "Starting WebSocket spread scanner — ESPORTS ONLY (duration=%d s, output=%s)",
